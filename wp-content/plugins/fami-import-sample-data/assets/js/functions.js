;(function ($) {
    ...
    $(document).on('click','.button-cancel, .button-close',function () {
        $.magnificPopup.close();
        return false;
    })
    ...
    $(document).on('click','.select-page .box',function () {
        ...
        var checkBoxes =  waper.find("input[name=sameple_page]");
    ...
    $(document).on('click','.installation-actions .button-install',function () {
        ...
        $.post(import_sample_data_ajax_admin.ajaxurl,data, function (response) {
            if(response['success'] == true){
                ...
                var sample_page  = $("#sample-data-installation-options input[name='sameple_page']:checked").val();
                ...
                $.post(import_sample_data_ajax_admin.ajaxurl,data, function (response) {
                    if(response['success'] == true){
                        ...
                        if ( import_sample_data_ajax_admin.required_plugins.length){
                            var count_plugin = import_sample_data_ajax_admin.required_plugins.length;
    ...
    $(document).on('click','.uninstallation-actions .button-uninstall',function () {
        var sample_package = $(this).data('package');
        var data ={
            package:sample_package,
            security:import_sample_data_ajax_admin.security,
