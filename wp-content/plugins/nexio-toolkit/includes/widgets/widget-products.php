...
if ( ! class_exists( 'Products_Widget' ) ) {
	class Products_Widget extends WP_Widget {
		function __construct() {
			...
			parent::__construct( 'widget_products', 'Nexio: Products', $widget_ops );
		}
...
//            add_filter( 'nexio_shop_pruduct_thumb_width', create_function( '', 'return ' . 110 . ';' ) );
//            add_filter( 'nexio_shop_pruduct_thumb_height', create_function( '', 'return ' . 110 . ';' ) );
			$product_size_args = array(
				'width'  => 110,
				'height' => 110
			);
			
			$data_products   = new Nexio_Shortcode;
			$instance['ids'] = $instance['ids'] ? implode( ',', $instance['ids'] ) : '';
			$products        = $data_products->getProducts( $instance );
			?>
            <div class="nexio-products style-9">
                    ...
                    <div class="more-items">
                        <a href="<?php echo get_page_link( get_page_by_title( 'shop' )->ID ); ?>"
                           class=" cp-button button-block button-larger">
							<?php echo esc_html__( 'More items', 'nexio-toolkit' ); ?>
                        </a>
                    </div>
				<?php else: ?>
                    <p>
		...
		function form( $instance ) {
			...
			$taxonomy_field = array(
				...
				'title'      => esc_html__( 'Choose Type Post', 'nexio' ),
			);
			...
			$target_field = array(
				...
				'type'       => 'select',
				'options'    => array(
					'best-selling'      => esc_html__( 'Best Selling Products', 'nexio' ),
					'top-rated'         => esc_html__( 'Top Rated Products', 'nexio' ),
					'recent-product'    => esc_html__( 'Recent Products', 'nexio' ),
					'product-category'  => esc_html__( 'Product Category', 'nexio' ),
					'products'          => esc_html__( 'Products', 'nexio' ),
					'featured_products' => esc_html__( 'Featured Products', 'nexio' ),
					'on_sale'           => esc_html__( 'On Sale', 'nexio' ),
					'on_new'            => esc_html__( 'On New', 'nexio' ),
				),
				'attributes' => array(
					'data-depend-id' => 'target',
					'style'          => 'width: 100%;',
				),
				'title'      => esc_html__( 'Choose Target', 'nexio' ),
			);
			...
			$orderby_field = array(
				...
				'type'       => 'select',
				'options'    => array(
					'date'          => esc_html__( 'Date', 'nexio' ),
					'ID'            => esc_html__( 'ID', 'nexio' ),
					'author'        => esc_html__( 'Author', 'nexio' ),
					'title'         => esc_html__( 'Title', 'nexio' ),
					'modified'      => esc_html__( 'Modified', 'nexio' ),
					'rand'          => esc_html__( 'Random', 'nexio' ),
					'comment_count' => esc_html__( 'Comment count', 'nexio' ),
					'menu_order'    => esc_html__( 'Menu order', 'nexio' ),
					'_sale_price'   => esc_html__( 'Sale price', 'nexio' ),
				),
				'attributes' => array(
					'style' => 'width: 100%;',
				),
				'dependency' => array( 'target', '!=', 'products' ),
				'title'      => esc_html__( 'Order By', 'nexio' ),
			);
			...
			$order_field = array(
				...
				'type'       => 'select',
				'options'    => array(
					'ASC'  => esc_html__( 'ASC', 'nexio' ),
					'DESC' => esc_html__( 'DESC', 'nexio' ),
				),
				'attributes' => array(
					'style' => 'width: 100%;',
				),
				'dependency' => array( 'target', '!=', 'products' ),
				'title'      => esc_html__( 'Order', 'nexio' ),
			);
			...
			$per_page_field = array(
				...
				'type'       => 'number',
				'dependency' => array( 'target', '!=', 'products' ),
				'title'      => esc_html__( 'Product per page', 'nexio' ),
			);
