...
class Nexio_Widgets_Instagram extends WP_Widget
{
    function __construct()
    {
        $widget_ops = array(
            'description' => esc_html__('Show off your favorite Instagram photos', 'nexio')
        );
        ...
        parent::__construct('nexio_instagram', esc_html__('Nexio: Instagram', 'nexio'), $widget_ops, $control_ops);
    }
    ...
    function widget($args, $instance)
    {
        ...
        if (intval($id) === 0) {
            echo '<p>' . esc_html__('No user ID specified.', 'nexio') . '</p>';
        }
        ...
        $output = '<div class="nexio-instagram-widget clearfix cols-' . esc_attr($columns) . '">';
    ...
    function form($instance)
    {
        ...
        <p>
            <label for="<?php echo esc_attr($this->get_field_id('limit')); ?>"><?php esc_html_e('Number of Photos:', 'nexio'); ?></label>
...
add_action('widgets_init', 'Nexio_Widgets_Instagram');
function Nexio_Widgets_Instagram()
{
    register_widget('Nexio_Widgets_Instagram');
}
