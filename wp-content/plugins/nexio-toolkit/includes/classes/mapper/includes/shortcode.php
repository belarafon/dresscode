...
class Nexio_Mapper_Shortcode {
	...
	public static function initialize() {
		// Add shortcode
		add_shortcode( 'nexio_mapper', array( __CLASS__, 'render_shortcode' ) );
	...
	public static function render_shortcode( $atts, $content = null ) {
		...
		// Get current image.
		$attachment_id = get_post_meta( $atts['id'], 'nexio_mapper_image', true );
		...
		$settings = get_post_meta( $atts['id'], 'nexio_mapper_settings', true );
		
		// Get all pins.
		$pins = get_post_meta( $atts['id'], 'nexio_mapper_pins', true );
		...
		// Generate HTML.
		$html .= '
		<div id="mapper-' . esc_attr( $atts['id'] ) . '" class="mapper ' . esc_attr( $settings['tooltip-style'] ) . ' ' . esc_attr( $settings['popup-show-effect'] ) . ' ' . esc_attr( $settings['image-effect'] ) . '" data-width="' . esc_attr( $image_data['width'] ) . '" data-height="' . esc_attr( $image_data['height'] ) . '">
		...
		if ( $pins ) {
			foreach ( $pins as $pin ) {
				...
				if ( class_exists( 'WooCommerce' ) && $pin['settings']['pin-type'] == 'woocommerce' && ( ! empty( $pin['settings']['product'] ) ) ) {
					...
					if ( $_product->get_price_html() ) {
						...
						if ( $pin['settings']['product-rate'] ) {
							...
							if ( $_product->get_rating_count() ) {
								...
								$html .= sprintf( __( '%s Reviews', 'nexio-toolkit' ), $_product->get_rating_count() );
								$html .= ')</div>';
							}
							$html .= '</div>';
						}
					...
					$html     .= '</div>';
					$img_size = nexio_toolkit_resize_image( get_post_thumbnail_id( $product_id ), null, 100, 100, true, true, false );
					...
					if ( $_product->is_type( 'simple' ) && ! Nexio_Mapper_Helper::yith_wc_product_add_ons( $product_id ) && ! Nexio_Mapper_Helper::check_gravityforms( $product_id ) && ! Nexio_Mapper_Helper::wc_measurement_price_calculator( $product_id ) ) {
						$html .= '<a href="' . esc_url( $_product->add_to_cart_url() ) . '" data-product_id="' . esc_attr( $product_id ) . '" data-quantity="1" class="ajax_add_to_cart add_to_cart_button button product_type_simple">' . __( 'Add To Cart', 'nexio-toolkit' ) . '</a>';
					} else {
						$html .= '<a href="' . esc_url( get_permalink( $product_id ) ) . '" data-product_id="' . esc_attr( $product_id ) . '" data-quantity="1" class="add_to_cart_button button product_type_simple">' . __( 'Select Options', 'nexio-toolkit' ) . '</a>';
					}
					$html .= '</div>';
					$html .= '</div>';
					...
					if ( ! empty( $pin['settings']['popup-title'] ) ) {
						...
						$html .= '<h3 class="csre-title">' . __( 'Add your title in backend', 'nexio-toolkit' ) . '</h3>';
					}
	...
	public static function enqueue_scripts() {
		...
		if ( has_shortcode( $post->post_content, "nexio_mapper" ) || has_shortcode( $post->post_excerpt, "nexio_mapper" ) || has_shortcode( $post->post_content, "nexio_pinmap" ) || has_shortcode( $post->post_excerpt, "nexio_pinmap" ) ) {
			// Enqueue required assets.
			wp_enqueue_style( 'nexio-toolkit', NEXIO_TOOLKIT_URL . '/includes/classes/mapper/assets/css/frontend.css' );
			wp_enqueue_script( 'nexio-toolkit', NEXIO_TOOLKIT_URL . '/includes/classes/mapper/assets/js/frontend.js', array(), false, true );
		}
	}
	...
	public static function inline_style( $id, $settings, $pins ) {
		...
		if ( $settings['popup-border-radius'] ) {
			$css .= '
			#mapper-' . $id . '.mapper .csre-pin .csre-popup-footer a {border-radius: 0 0 ' . (int) $settings['popup-border-radius'] . 'px ' . (int) $settings['popup-border-radius'] . 'px;}';
		}
		...
		if ( $pins ) {
			foreach ( $pins as $pin ) {
				// Popup width & height
				$css .= '
			#mapper-' . $id . ' #csre-pin-' . esc_attr( $pin['settings']['id'] ) . ' .csre-popup {';
				...
				// Pin style setting
				$css .= '
			#mapper-' . $id . ' #csre-pin-' . esc_attr( $pin['settings']['id'] ) . ' .icon-pin {';
				...
				// Pin hover setting
				$css .= '
			#mapper-' . $id . ' #csre-pin-' . esc_attr( $pin['settings']['id'] ) . ' .icon-pin:hover {';
				...
				// Hover text
				$css .= '
                #mapper-' . $id . ' #csre-pin-' . esc_attr( $pin['settings']['id'] ) . ' .text__area:hover,
                 #mapper-' . $id . ' #csre-pin-' . esc_attr( $pin['settings']['id'] ) . ' img:hover{ ';
