jQuery(function ($) {
    ...
    var initTab = function () {
        $(document).on('click', '.mapper ul.nav li', function (e) {
    ...
    var imageSelector = function () {
        $(document).on('click', '.mapper a.btn-add-image, .mapper #change-image, .mapper a.image-selector', function (event) {
            ...
            if (!window.wr_image_selector) {
                ...
                window.wr_image_selector = wp.media({
                    button: {
                        text: nexio_mapper.text.img_selector_btn_label,
                    },
                    states: [new wp.media.controller.Library({
                        title: nexio_mapper.text.img_selector_modal_title,
                ...
                window.wr_image_selector.on('select', function () {
                    ...
                    if (window.wr_image_selector.input_element.attr('id') == 'nexio_mapper_image') {
                        window.wr_image_selector.input_element.val(attachment.attributes.id);

                        // Update edit area with new image.
                        if ($('.mapper-bot > .edit-image').length) {
                            $('.mapper-bot .image-wrap > img').attr('src', attachment.attributes.url);
                        } else {
                            var edit_html = $('#nexio_mapper_image_tmpl').text().replace('%URL%', attachment.attributes.url);

                            $('.mapper-bot > .add-image').replaceWith(edit_html);
            ...
            window.wr_image_selector.input_element = $(this).hasClass('image-selector') ? $(this).prev('input') : $('#nexio_mapper_image');
    ...
    var setupApplication = function () {
        ...
        var PinList = Backbone.Collection.extend({
            ...
            fetch: function () {
                if (window.nexio_mapper_pins && window.nexio_mapper_pins.length) {
                    this.add(window.nexio_mapper_pins);
                }
            },
        });
        ...
        var PinView = Backbone.View.extend({
            ...
            template: _.template($('#nexio_mapper_pin_tmpl').text()),
            ...
            render: function () {
                ...
                this.$el.find('[data-option]').each(function () {
                    ...
                    if ($(this).attr('name') != option[0]) {
                        if (option[3] !== undefined) {
                            $(this).attr('name', 'nexio_mapper_pins[' + self.index + '][' + option[1] + '][' + option[3] + ']');
                        } else {
                            $(this).attr('name', 'nexio_mapper_pins[' + self.index + '][' + option[1] + ']');
                        }
                    }
                    ...
                    if ($(this).hasClass('color-picker')) {
                        ...
                        setTimeout($.proxy(function () {
                            $(this).parent().parent().click(function () {
                                ...
                                // focused.
                                $('.mapper .icon-selector').each(function () {
                    ...
                    // Init icon selector if needed.
                        ...
                        $(this).addClass('hidden').after($('#nexio_mapper_icon_selector_tmpl').text().replace('%SELECTED%', $(this).val()));
                    }
                    ...
                    // Init product selector if needed.
                        ...
                        $(this).select2({
                            ...
                            ajax: {
                                url: nexio_mapper.product_selector.url,
                                dataType: 'json',
                                delay: 250,
                                data: function (terms) {
                                    return {
                                        term: terms,
                                        security: nexio_mapper.product_selector.security,
                                    };
                                },
                            ...
                            initSelection: function (element, callback) {
                                ...
                                if (id !== '') {
                                    $.ajax(nexio_mapper.product_selector.url + '&term=' + id + '&security=' + nexio_mapper.product_selector.security, {
                                        dataType: 'json',
                                    }).done(function (data) {
                                        callback(data);
                                    });
                ...
                }).on('click', '.nexio-icon-list a', function (event) {
                    event.preventDefault();
            ...
            edit: function (event) {
                ...
                if (this.$el.hasClass('editing')) {
                    ...
                    if (!this.$el.data('nexio_mapper_pin_settings_initialized')) {
                        var self = this;
                        ...
                        this.$el.data('nexio_mapper_pin_settings_initialized', true);
                    }
            ...
            close: function (event) {
                ...
                if (pin_type == 'woocommerce') {
                    ...
                    this.$el.find('.tooltip').text(title ? title : nexio_mapper.text.please_input_a_title).removeClass('hidden');
                }
            ...
            remove: function (event) {
                ...
                if (confirm(nexio_mapper.text.confirm_removing_pin)) {
                    this.$el.remove();
                    this.model.destroy();
        ...
        var PinListView = Backbone.View.extend({
            ...
            el: '.mapper-bot > .edit-image > .image-wrap',
            ...
            initialize: function () {
                ...
                $(document).on('click', '.mapper-bot > .edit-image > .image-wrap > img', this.create);
        ...
        $(document).on('init_nexio_mapper', function (event) {
            if (!$(document).data('nexio_mapper_settings_initialized')) {
                ...
                $('#general-settings').find('input, select, textarea').each(function () {
                    if (window.nexio_mapper_settings[$(this).attr('name')]) {
                        $(this).val(window.nexio_mapper_settings[$(this).attr('name')]);

                        if ($(this).attr('type') == 'hidden' && $(this).next().attr('type') == 'checkbox') {
                            if (parseInt(window.nexio_mapper_settings[$(this).attr('name')])) {
                                $(this).next().attr('checked', 'checked');
                            } else {
                                $(this).next().removeAttr('checked');
                            }
                    ...
                    $(this).attr('name', 'nexio_mapper_settings[' + $(this).attr('name') + ']');
                ...
                $('#general-settings').on('change', 'input[type="radio"], select', function () {
                    var option = $(this).attr('name').match(/nexio_mapper_settings\[([^\]]+)\]/), value = $(this).val();
                ...
                $(document).click(function (event) {
                    ...
                    $('.mapper .wp-picker-holder .iris-picker').each(function () {
                ...
                // Track data changes.
                var data_fields = '.mapper input, .mapper select, .mapper textarea';

                $(document).on('change', data_fields, function () {
                    if ($(this).attr('name') && $(this).attr('name').indexOf('nexio_mapper_') > -1) {
                        $('#post #publish').attr('data-changed', 'yes');
                    }
                });
                ...
                $(window).on('beforeunload', function () {
                    ...
                    if (submit_btn.length && submit_btn.attr('data-changed') == 'yes') {
                        return nexio_mapper.text.ask_for_saving_changes;
                    }
                });

                $(document).data('nexio_mapper_settings_initialized', true);
            }

            if (!window.nexio_mapper_pins_app && $('.mapper-bot > .edit-image > .image-wrap').length) {
                // Init pin list view.
                window.nexio_mapper_pins_app = new PinListView;
            }
        });
    };
