<?php
if ( !class_exists( 'Nexio_Attribute_Product_Meta' ) ) {
	class Nexio_Attribute_Product_Meta {
		public $plugin_uri;
		public $screen;
		public $taxonomy;
		...
		public function product_attributes_type_selector( $types ) {
			$nexio_types = array(
				'box_style' => esc_html__( 'Box Style', 'nexio' ),
			);
			
			return array_merge( $types, $nexio_types );
		}
		...
		public function add_attribute_fields() {
            ...
            <div class="form-field" style="display: none;">
                <label><?php esc_html_e( 'Size Box', 'nexio' ); ?></label>
                <input type="text" name="attribute_size" id="attribute_size" value="30x30">
                <p class="description"><?php esc_html_e( 'Determines Size in this attribute.{width}x{height}', 'nexio' ); ?></p>
            </div>
		...
		public function edit_attribute_fields() {
            ...
            <tr class="form-field form-required" <?php if ( $attribute->attribute_type == 'select' ): ?> style="display: none;" <?php endif; ?>>
                <th scope="row" valign="top"><label><?php _e( 'Size Box', 'nexio' ); ?></label></th>
                <td>
		...
		public function product_option_terms( $attribute_taxonomy, $i ) {
			...
			if ( 'box_style' === $attribute_taxonomy->attribute_type ) : ?>
                <select multiple="multiple" data-placeholder="<?php esc_attr_e( 'Select terms', 'nexio' ); ?>"
                ...
                <button class="button plus select_all_attributes"><?php esc_html_e( 'Select all', 'nexio' ); ?></button>
                <button class="button minus select_no_attributes"><?php esc_html_e( 'Select none', 'nexio' ); ?></button>
                <button class="button fr plus add_new_attribute"><?php esc_html_e( 'Add new', 'nexio' ); ?></button>
				<?php
			endif;
		}
		...
		public function add_attr_field() {
            ...
            <div class="form-field term-display-type-wrap">
                <label for="display_type"><?php _e( 'Display type', 'nexio' ); ?></label>
                <select name="<?php echo esc_attr( $id_type ); ?>" id="product_attribute_type_id">
                    <option value="-1"><?php echo esc_html__( 'None', 'nexio' ); ?></option>
                    <option value="color"><?php echo esc_html__( 'Color', 'nexio' ); ?></option>
                    <option value="photo" selected><?php echo esc_html__( 'Photo', 'nexio' ); ?></option>
                    <option value="label"><?php echo esc_html__( 'Label', 'nexio' ); ?></option>
                </select>
                <script type="text/javascript">
            ...
            <div class="form-field term-display-type-wrap swatch photo">
                <label><?php _e( 'Thumbnail', 'nexio' ); ?></label>
                ...
                <div style="line-height: 60px;">
                    ...
                    <button type="button"
                            class="upload_image_button button"><?php _e( 'Upload/Add image', 'nexio' ); ?></button>
                    <button type="button"
                            class="remove_image_button button"><?php _e( 'Remove image', 'nexio' ); ?></button>
                </div>
                <script type="text/javascript">
                    ...
                    jQuery(document).on('click', '.upload_image_button', function (event) {
                        ...
                        file_frame = wp.media.frames.downloadable_file = wp.media({
                            title: '<?php _e( 'Choose an image', 'nexio' ); ?>', button: {
                                text: '<?php _e( 'Use image', 'nexio' ); ?>'
                            }, multiple: false
                        });
		...
		public function edit_attr_field( $term, $taxonomy ) {
			...
			$id_color    = "product_attribute_meta[{$this->meta_key}][color]";
			$id_photo    = "product_attribute_meta[{$this->meta_key}][photo]";
			$swatch_term = new Nexio_Term( $this->meta_key, $term->term_id, $taxonomy, false );
            ...
            <tr class="form-field swatch color <?php echo $swatch_term->get_type() == 'color' ? 'active' : ''; ?>">
                <th scope="row" valign="top"><label><?php _e( '', 'nexio' ); ?></label></th>
                <td>
		...
		public function product_attr_columns( $columns ) {
			...
			$new_columns['thumb'] = __( 'Image', 'nexio' );
		...
		public function product_attr_column( $columns, $column, $id ) {
			if ( 'thumb' === $column ) {
				$swatch_term = new Nexio_Term( $this->meta_key, $id, $this->taxonomy, false );
		...
		public function wc_variation_attribute_options( $html, $args ) {
			...
			$args                  = wp_parse_args( apply_filters( 'woocommerce_dropdown_variation_attribute_options_args', $args ), array(
				                                                                                                                       ...
				                                                                                                                       'id'               => '',
				                                                                                                                       'class'            => '',
				                                                                                                                       'show_option_none' => __( 'Choose an option', 'nexio' ),
			                                                                                                                       )
			);
			...
			$show_option_none_text = $args['show_option_none'] ? $args['show_option_none'] : __( 'Choose an option', 'nexio' ); // We'll do our best to hide the placeholder, but we'll need to show something when resetting options.
		...
		public function wc_loop_variation_attribute_options() {
			global $product;
			$products_loop_attributes_display = nexio_get_option( 'products_loop_attributes_display', array() );
			...
			if ( $product->get_type() == 'variable' ) {
				...
				foreach ( $available_variations as $available_variation ) {
					$image_variable                            = nexio_toolkit_resize_image( $available_variation['image_id'], null, $width, $height, $crop, true, false );
		...
		public function get_product_attribute( $attribute ) {
			...
			try {
				...
				if ( is_wp_error( $attribute ) || is_null( $attribute ) ) {
					throw new WC_API_Exception( 'woocommerce_api_invalid_product_attribute_id', __( 'A product attribute with the provided ID could not be found', 'nexio' ), 404 );
				}
	...
	new Nexio_Attribute_Product_Meta();
}