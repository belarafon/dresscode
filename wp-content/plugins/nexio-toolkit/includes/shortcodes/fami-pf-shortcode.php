...
'use strict';
	...
	public static function prdctfltr_sc_products_700( $atts, $content = null ) {
		...
		if ( $atts['use_filter'] == 'yes' ) {
			$filter_icon_html = '<div class="part-wrap part-acrion-left-wrap">
                                    <div class="part-filter-wrap">
                                        <div class="actions-wrap">
                                            <a class="filter-toggle filter-toggle-button" href="#">
                                            <i class="flaticon-filter"></i>' . esc_html__( 'Filter', 'nexio-toolkit' ) . '</a>
                                        </div>
                                    </div>
                                </div>';
		}
		...
		if ( trim( $categories_display ) != '' ) {
			...
			if ( $is_real_mobile ) {
				...
				$owl_item_close  = ''; // '</div>';
				$cat_link_html   = $owl_item_open . '<span data-cat_slug="" class="active all-cats cat-item"><a href="' . esc_url( get_permalink( $shop_page_id ) ) . '">' . esc_html__( 'All', 'nexio-toolkit' ) . '</a></span>';
				...
				if ( $cats_display_html != '' ) {
					if ( $shop_page_id ) {
						$shop_link_html    = '<li data-cat_slug="" class="active all-cats"><a href="' . esc_url( get_permalink( $shop_page_id ) ) . '" title="' . esc_attr__( 'Shop', 'nexio-toolkit' ) . '">' . esc_html__( 'All', 'nexio-toolkit' ) . '</a></li>';
		...
		$enable_products_sizes = nexio_toolkit_get_option( 'enable_products_sizes', false );
		...
		$top_bar_output = '<div class="toolbar-products toolbar-top toolbar-products-shortcode ' . esc_attr( $toolbar_class ) . '"><div class="nexio-container">' . $filter_icon_html . '<div class="part-wrap part-acrion-right-wrap">'. $products_sizes_html . '</div></div></div>';
		$content_output = '<div class="fami-products-shortcode-wrap nexio-container">' . $cached . $cache_products . $cache_pagination . '</div>';
