jQuery(document).ready(function ($) {
    ...
    $(document).on('click', '.img-item .ziss-add-image', function (e) {
        ...
        var added_item_html = '<div class="used-img-item-wrap col-md-3"><div class="used-img-item hover-zoom-img"><figure data-instagram_data="' + encodeURIComponent(instagram_data) + '" data-pin_data="" data-src="' + img_src + '" data-width="' + img_w + '" data-height="' + img_h + '" data-social_source="' + social_source + '" style="background-image: url(' + img_src + ');"></figure>' + remove_img_html + '</div></div>';
    ...
    $(document).on('click', '.ziss-used-imgs .used-img-item > figure', function (e) {
        ...
        if (!$.isEmptyObject(instagram_data)) {
            ...
            if (!popupContentRight.find('.ziss-img-caption').length && img_caption_text != '') {
                popupContentRight.append('<div class="ziss-img-caption"></div>');
            }
            if (img_caption_text != '') {
                popupContentRight.find('.ziss-img-caption').html(img_caption_text);
            }
        }
    ...
    function ziss_update_chosen() {
        console.log('go here 1');
        if (!$('.ziss-select:not(.ziss-chosen-loaded)').length) {
            return false;
        }
    
        console.log('I am here 1');
        
        $('.ziss-select:not(.ziss-chosen-loaded)').each(function () {
            $(this).addClass('ziss-chosen-loaded').chosen();
        });
    }
    ...
    function ziss_update_social_shop_pin_data() {
        ...
        $('.ziss-used-imgs .used-img-item figure').each(function () {
            ...
            if ($.trim(instagram_data) != '') {
                instagram_data = JSON.parse(decodeURIComponent(instagram_data));
            }
            else {
                instagram_data = {};
            }
