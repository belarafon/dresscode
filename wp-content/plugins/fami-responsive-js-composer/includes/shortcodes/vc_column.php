<?php
    ...
    if( !class_exists('Responsive_Js_Composer_Vc_column')){
        class Responsive_Js_Composer_Vc_column extends Responsive_Js_Composer_Shortcode{
            ...
            public static function generate_css( $atts ) {
                ...
                if( isset( $atts['enbale_extend_reponsive'] ) && $atts['enbale_extend_reponsive'] =='yes'){
                    $vc_column_reponsive = array();
                    if( isset($atts['vc_column_reponsive'])){
                        $vc_column_reponsive = vc_param_group_parse_atts( $atts['vc_column_reponsive'] );
                    }


                    if( $vc_column_reponsive && count($vc_column_reponsive) > 0 ){
                        foreach ($vc_column_reponsive as $item ){
                            ...
                            if( $screen !='' && is_numeric($screen) && $screen > 0 && $item['value']!= "" ){
                                if( $unit =='other'){
                                    $css .='@media (max-width: '.$screen.'px){ .'.$atts['responsive_js_composer_custom_id'].' { width:'.$item['value'].'!important;} }';
                                }else{
                                    $css .='@media (max-width: '.$screen.'px){ .'.$atts['responsive_js_composer_custom_id'].' { width:'.$item['value'].$unit.'!important;} }';
                                }
            ...
            public function add_param(){
                $attributes = array(
                    ...
                    array(
                        'type' => 'param_group',
                        'value' => '',
                        'param_name' => 'vc_column_reponsive',
        ...
        new Responsive_Js_Composer_Vc_column();
    }