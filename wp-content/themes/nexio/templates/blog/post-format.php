	...
	<?php if ( $images ) : ?>
        <div class="post-format post-gallery owl-carousel nav-center" data-slidespeed="800" data-autoplay="true"
			...
			<?php foreach ( $images as $image_id ) : ?>
				<?php
				$image          = nexio_resize_image( $image_id, null, 1170, 780, true, true );
				$imgage_caption = get_post_field( 'post_excerpt', $image_id );
				?>
                <div class="slide-item">
					<?php echo nexio_img_output( $image, '', '', $imgage_caption ); ?>
                </div>
			<?php endforeach; ?>
        </div>
	<?php endif; ?>
        ...
        <div class="post-format post-image">
			...
			<?php else : ?>
                <div class="post-thumb">
                    <figure class="single-media"><?php nexio_post_thumbnail(); ?></figure>
                </div>
			<?php endif; ?>
        </div>
	<?php endif; ?>
<?php else : // Post format standard... ?>
	<?php if ( has_post_thumbnail() ): ?>
        <div class="post-format post-<?php echo esc_attr( $post_format ); ?>">
			<?php if ( ! is_single() ) : ?>
                <a href="<?php the_permalink(); ?>"><?php nexio_post_thumbnail( $width, $height ); ?></a>
			<?php else: ?>
                <div class="post-thumb">
                    <figure class="single-media"><?php nexio_post_thumbnail(); ?></figure>
                </div>
