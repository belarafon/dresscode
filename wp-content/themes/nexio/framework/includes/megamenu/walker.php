<?php
/**
 * Class Name: nexio_bootstrap_navwalker
 ...
 */
if ( ! class_exists( 'Nexio_navwalker' ) ) {
	class Nexio_navwalker extends Walker_Nav_Menu {
		/**
						...
						if ( $this->megamenu_img_icon ) {
							...
							if ( $imgicon ) {
								...
								if ( $imgicon_ext == 'svg' ) {
									...
									$item_output .= '<span class="logo">' . nexio_img_output( $imgicon_arg, 'image_icon_1' ) . '</span>';
								} else {
		...
		public static function fallback( $args ) {
			if ( current_user_can( 'manage_options' ) ) {
				...
				$fb_output .= '>';
				$fb_output .= '<li><a href="' . admin_url( 'nav-menus.php' ) . '">' . esc_html__( 'Add a menu', 'nexio' ) . '</a></li>';
				$fb_output .= '</ul>';
				if ( $container ) {
